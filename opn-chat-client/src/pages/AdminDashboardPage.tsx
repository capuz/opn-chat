...
const LivePanel = () => {
  ...
  useEffect(() => {
    const load = () => adminService.getLiveData().then(setLive).catch(() => {});
    load();
    const id = setInterval(load, 5000);
    return () => clearInterval(id);
  }, []);

  if (!live) return <div style={loadingStyle()}>Connecting to live feed...</div>;

  return (
    <div style={panelStyle()}>
      <h2 style={h2Style()}>Live Monitoring <span style={{ fontSize: 12, color: 'var(--ch-text-3)' }}>(auto-refresh 5s)</span></h2>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, height: 'calc(100% - 60px)' }}>
...
const UsersPanel = () => {
  ...
  const load = useCallback((p: number, s: string) => {
    adminService.getUsers(p, 20, s || undefined).then(setResult).catch(() => {});
  }, []);
...
const RoomsPanel = () => {
  ...
  const load = () => adminService.getRooms().then(setRooms).catch(() => {});
  useEffect(() => { load(); }, []);
...
const ReportsPanel = () => {
  ...
  const load = useCallback((p = 1) => {
    adminService.getReports(unresolvedOnly, p, 20).then(setResult).catch(() => {});
  }, [unresolvedOnly]);
...
const AuditPanel = () => {
  ...
  const load = useCallback((p = 1) => {
    adminService.getAuditLogs({ ...params, page: p, pageSize: 30 }).then(setResult).catch(() => {});
  }, [params]);
...
const AnalyticsPanel = () => {
  ...
  useEffect(() => { adminService.getAnalytics().then(setData).catch(() => {}); }, []);
...
const SettingsPanel = () => {
  ...
  useEffect(() => { adminService.getSettings().then(setSettings).catch(() => {}); }, []);

  const update = (key: string, value: string) => setSettings(prev => prev.map(s => s.key === key ? { ...s, value } : s));
  const getVal = (key: string) => settings.find(s => s.key === key)?.value ?? '';

  const save = () => adminService.updateSettings(settings).then(() => { setSaved(true); setTimeout(() => setSaved(false), 2000); }).catch(() => alert('Save failed'));
    ...
    <div style={panelStyle()}>
      ...
      <div style={{ maxWidth: 520, display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div style={settingRow()}>
          ...
          <input type="number" min={0} max={99} value={getVal('MaxNicknameChanges')} onChange={e => update('MaxNicknameChanges', e.target.value)} style={inputStyle()} />
        </div>
        <div style={settingRow()}>
          <label style={settingLabel()}>Allow Private Chats</label>
          <Toggle value={getVal('AllowPrivateChats') === 'true'} onChange={v => update('AllowPrivateChats', v ? 'true' : 'false')} label="" />
        </div>
        <div style={settingRow()}>
          <label style={settingLabel()}>Allow Room Creation</label>
          <Toggle value={getVal('AllowRoomCreation') === 'true'} onChange={v => update('AllowRoomCreation', v ? 'true' : 'false')} label="" />
        </div>
        <div style={settingRow()}>
          <label style={settingLabel()}>Spam Threshold (msg/min)</label>
          <input type="number" min={1} max={100} value={getVal('SpamThreshold')} onChange={e => update('SpamThreshold', e.target.value)} style={inputStyle()} />
        </div>
        <div style={settingRow()}>
          <label style={settingLabel()}>Global Announcement Banner</label>
          <input value={getVal('GlobalAnnouncementBanner')} onChange={e => update('GlobalAnnouncementBanner', e.target.value)} placeholder="Empty = no banner" style={{ ...inputStyle(), flex: 1 }} />
        </div>
        <div style={{ ...settingRow(), background: maintenance ? 'rgba(239,68,68,0.08)' : 'transparent', borderRadius: 6, padding: 10, border: maintenance ? '1px solid #ef444440' : '1px solid transparent' }}>
          <label style={{ ...settingLabel(), color: maintenance ? '#ef4444' : undefined }}>Maintenance Mode</label>
